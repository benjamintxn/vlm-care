...
def build_model():
    ...
    for i in range(n_frames):
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
            ...
            alphas = tf.nn.softmax(tf.reduce_sum(tf.matmul(e, brcst_w), 2), 0) * zeros_object[i]
            attention = tf.reduce_sum(tf.expand_dims(alphas, 2) * n_object, 0)
            ...
            soft_pred = (tf.reshape(
                tf.gather(tf.transpose(tf.nn.softmax(pred), (1, 0)), 1),
                (batch_size, 1)
            ) if i == 0 else tf.concat([
                soft_pred,
                tf.reshape(tf.gather(tf.transpose(tf.nn.softmax(pred), (1, 0)), 1), (batch_size, 1))
            ], 1))
            all_alphas = (tf.expand_dims(alphas, 0) if i == 0 else tf.concat([all_alphas, tf.expand_dims(alphas, 0)], 0))

            pos_loss = -tf.multiply(
                tf.exp(-(n_frames - i - 1) / 20.0),
                -tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y)
            )
            neg_loss = tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=pred)
            temp_loss = tf.reduce_mean(
                pos_loss * y[:, 1] + neg_loss * y[:, 0]
            )
            loss += temp_loss

    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss / n_frames)
...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        epoch_loss = []
        np.random.shuffle(np.arange(1, train_num+1))
        t0 = time.time()

        for batch in range(1, train_num+1):
            data = np.load(f"{train_path}batch_{batch:03d}.npz")
            batch_xs, batch_ys = data['data'], data['labels']
            _, batch_loss = sess.run([
                optimizer, loss
            ], feed_dict={x: batch_xs, y: batch_ys, keep: [0.5]})
            epoch_loss.append(batch_loss / batch_size)
            print(f"  Batch {batch}/{train_num} 	 Loss: {batch_loss/batch_size:.4f}")

        avg = np.mean(epoch_loss)
        print(f"Epoch {epoch+1} complete. Avg Loss: {avg:.4f} 	 Time: {time.time()-t0:.2f}s")
